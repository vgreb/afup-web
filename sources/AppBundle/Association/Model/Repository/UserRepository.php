...
class UserRepository extends Repository implements MetadataInitializer, UserProviderInterface, PasswordUpgraderInterface
{
    ...
    public function loadActiveUsersByCompany(int $companyId)
    {
        $queryBuilder = $this->getQueryBuilderWithCompleteUser();
        $queryBuilder
            ->where('app.id_personne_morale = :company')
    ...
    private function getQueryBuilderWithSubscriptions()
    {
        ...
        $queryBuilder
            ->cols([
                ...
                'app.`email`', 'app.`id_personne_morale`',
            ])
            ->from('afup_personnes_physiques app')
            ->join('LEFT', 'afup_cotisations ac', 'ac.type_personne = IF(app.id_personne_morale = 0, 0, 1) AND ac.id_personne = IF(app.id_personne_morale = 0, app.id, app.id_personne_morale)')
            ->groupBy(['app.`id`'])
        ;
